...
export function restoreReducer(state, action) {
    ...
    const restoreSeed = convertServerPatchToFullTree(treeToRestore, null, renderedSearch);
    const task = startPPRNavigation(now, currentUrl, state.renderedSearch, state.cache, state.tree, restoreSeed.routeTree, restoreSeed.metadataVaryPath, FreshnessPolicy.HistoryTraversal, null, null, false, accumulation);
    if (task === null) {
        const mutable = {
