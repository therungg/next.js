...
import { convertServerPatchToFullTree, navigateToKnownRoute, navigate as navigateUsingSegmentCache } from '../../segment-cache/navigation';
 ...
 */ export function serverActionReducer(state, action) {
    ...
    return fetchServerAction(state, nextUrl, action).then(async ({ revalidationKind, actionResult, actionFlightData: flightData, actionFlightDataRenderedSearch: flightDataRenderedSearch, redirectLocation, redirectType })=>{
        ...
        if (flightData !== undefined && flightDataRenderedSearch !== undefined) {
            ...
            const redirectSeed = convertServerPatchToFullTree(currentFlightRouterState, flightData, flightDataRenderedSearch);
            const now = Date.now();
            const result = navigateToKnownRoute(now, redirectUrl, redirectCanonicalUrl, redirectSeed, currentUrl, currentRenderedSearch, state.cache, currentFlightRouterState, freshnessPolicy, nextUrl, shouldScroll);
        ...
        const result = navigateUsingSegmentCache(redirectUrl, currentUrl, currentRenderedSearch, state.cache, currentFlightRouterState, nextUrl, freshnessPolicy, shouldScroll, mutable);
        return handleNavigationResult(redirectUrl, state, mutable, pendingPush, result);
    }, (e)=>{
