...
export function serverPatchReducer(state, action) {
    ...
    const now = Date.now();
    const result = navigateToKnownRoute(now, retryUrl, retryCanonicalUrl, retrySeed, currentUrl, currentRenderedSearch, state.cache, state.tree, FreshnessPolicy.RefreshAll, retryNextUrl, shouldScroll);
