...
import { EntryStatus, readRouteCacheEntry, requestOptimisticRouteCacheEntry, convertRootFlightRouterStateToRouteTree } from './cache';
 ...
 */ export function navigate(url, currentUrl, currentRenderedSearch, currentCacheNode, currentFlightRouterState, nextUrl, freshnessPolicy, shouldScroll, accumulation) {
    const now = Date.now();
    const href = url.href;
...
export function navigateToKnownRoute(now, url, canonicalUrl, navigationSeed, currentUrl, currentRenderedSearch, currentCacheNode, currentFlightRouterState, freshnessPolicy, nextUrl, shouldScroll) {
    ...
    const task = startPPRNavigation(now, currentUrl, currentRenderedSearch, currentCacheNode, currentFlightRouterState, navigationSeed.routeTree, navigationSeed.metadataVaryPath, freshnessPolicy, navigationSeed.data, navigationSeed.head, isSamePageNavigation, accumulation);
    if (task !== null) {
...
async function navigateToUnknownRoute(now, url, currentUrl, currentRenderedSearch, nextUrl, currentCacheNode, currentFlightRouterState, freshnessPolicy, shouldScroll, collectedDebugInfo) {
    ...
    return navigateToKnownRoute(now, url, createHrefFromUrl(canonicalUrl), navigationSeed, currentUrl, currentRenderedSearch, currentCacheNode, currentFlightRouterState, freshnessPolicy, nextUrl, shouldScroll);
}
