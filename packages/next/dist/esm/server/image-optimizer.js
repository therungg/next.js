...
export class ImageOptimizerCache {
    ...
    async get(cacheKey) {
        ...
        try {
            ...
            for (const file of files){
                ...
                const buffer = await promises.readFile(/* turbopackIgnore: true */ join(/* turbopackIgnore: true */ cacheDir, file));
    ...
    async set(cacheKey, value, { cacheControl }) {
        ...
        try {
            await writeToCacheDir(join(/* turbopackIgnore: true */ this.cacheDir, cacheKey), value.extension, revalidate, expireAt, value.buffer, value.etag, value.upstreamEtag);
        } catch (err) {
