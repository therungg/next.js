...
import { createHangingInputAbortSignal, throwToInterruptStaticGeneration } from '../app-render/dynamic-rendering';
...
async function generateCacheEntryImpl(workStore, cacheContext, innerCacheStore, clientReferenceManifest, encodedArguments, fn, timeoutError) {
    ...
    const handleError = createReactServerErrorHandler(workStore.dev, workStore.isBuildTimePrerendering ?? false, workStore.reactServerErrorsByDigest, (error)=>{
            ...
            case 'request':
                {
                    if (process.env.NODE_ENV === 'development') {
                        ...
                        await makeDevtoolsIOAwarePromise(undefined, outerWorkUnitStore, RenderStage.Runtime);
                    }
                    break;
                }
            case 'private-cache':
                    ...
                    switch(workUnitStore.type){
                        case 'prerender':
                            ...
                            if (existingEntry.revalidate === 0) {
                                ...
                                debug == null ? void 0 : debug('omitting entry', serializedCacheKey, 'from static shell due to short expire value:', existingEntry.expire);
                            }
                            if (cacheSignal) {
                                cacheSignal.endRead();
                            }
                        ...
                        case 'request':
                            {
                                if (process.env.NODE_ENV === 'development') {
                                    ...
                                    await makeDevtoolsIOAwarePromise(undefined, workUnitStore, RenderStage.Runtime);
                                }
                                break;
                            }
                        case 'prerender-legacy':
                ...
                if (existingEntry.stale < RUNTIME_PREFETCH_DYNAMIC_STALE) {
                    switch(workUnitStore.type){
                        case 'prerender-runtime':
                            ...
                            debug == null ? void 0 : debug('omitting entry', serializedCacheKey, 'from runtime shell due to short stale value:', existingEntry.stale);
                            if (cacheSignal) {
                                cacheSignal.endRead();
                            }
                            return makeHangingPromise(workUnitStore.renderSignal, workStore.route, 'dynamic "use cache"');
            ...
            switch(workUnitStore.type){
                case 'prerender':
                    ...
                    if (entry.revalidate === 0) {
                        ...
                        debug == null ? void 0 : debug('omitting entry', serializedCacheKey, 'from static shell due to short expire value:', entry.expire);
                    }
                    if (cacheSignal) {
                        cacheSignal.endRead();
                    }
