...
export default class Server {
   ...
   */ this.normalize = (pathname)=>{
        ...
        this.isCacheComponentsEnabled = this.enabledDirectories.app && !!this.nextConfig.cacheComponents;
        this.normalizers = {
        ...
        try {
            ...
            if (useMatchedPathHeader) {
                try {
                    ...
                    if (this.isCacheComponentsEnabled && this.minimalMode && req.headers[NEXT_RESUME_HEADER] === '1' && req.method === 'POST') {
                        // Get the configured max postponed state size.
                        const maxPostponedStateSize = this.nextConfig.experimental.maxPostponedStateSize ?? DEFAULT_MAX_POSTPONED_STATE_SIZE;
                        const maxPostponedStateSizeBytes = parseMaxPostponedStateSize(this.nextConfig.experimental.maxPostponedStateSize);
   ...
   */ async prepare() {
    ...
    async pipeImpl(fn, partialContext) {
        ...
        if (!res.sent) {
            ...
            if (dev) {
                res.setHeader('Cache-Control', this.nextConfig.experimental.devCacheControlNoCache ? 'no-cache, must-revalidate' : 'no-store, must-revalidate');
     ...
     */ const couldSupportPPR = this.isCacheComponentsEnabled && typeof routeModule !== 'undefined' && isAppPageRouteModule(routeModule);
